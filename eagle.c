...
static vector_t *vcf_read(FILE *file)
{
    ...
    while ((read_file = getline(&line, &line_length, file)) != -1)
    {
        ...
        for (s1 = ref; sscanf(s1, "%[^, ]%n", ref_token, &n1) == 1 || sscanf(s1, "%[-]%n", ref_token, &n1) == 1; s1 += n1 + 1)
        { // heterogenenous non-reference (comma-delimited) as separate entries
...
static fasta_t *refseq_fetch(char *name, const char *fa_file)
{
    ...
    if (absent)
    {
        kh_val(refseq_hash, k) = f;
    }
    else
    {
        exit_err("# refseq_hash collision: %s", asctime(time_info));
    }
    fai_destroy(fai);
    pthread_mutex_unlock(&refseq_lock);
    return f;
...
static char *construct_altseq(const char *refseq, int refseq_length, const vector_int_t *combo, variant_t **var_data, int *altseq_length)
{
    ...
    *altseq_length = refseq_length;
    for (i = 0; i < combo->len; i++)
    {
        ...
        if (pos < 0 || pos > *altseq_length)
        {
            exit_err("Variant at %s:%d is out of bounds in reference\n", v->chr, v->pos);
        }

        char *var_ref, *var_alt;
        if (v->ref[0] == '-')
        { // account for "-" variant representations
            var_ref = "";
            var_alt = v->alt;
        }
        else if (v->alt[0] == '-')
        ...
        else
        {
            ...
            char *s2 = v->alt;
            while (*s1 == *s2)
            { // account for and disregard common prefix in variant
                s1++;
                s2++;
                pos++;
            }
...
static inline int variant_find(const vector_int_t *a, int v)//binary search
{
    int i = 0;
    int j = a->len - 1;
    int n = (i + j) / 2;
...
static inline void variant_print(char **output, const vector_t *var_set, int i, int nreads, int not_alt_count, int has_alt_count, double total, double has_alt, double not_alt)
{
    ...
    else  snprintf(token, n, "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%e\t%f\tV\t", var_data[i]->chr, var_data[i]->pos, var_data[i]->ref, var_data[i]->alt, alt_tmp+ref_tmp, ref_tmp, alt_tmp, prob, odds);
...
static void calc_likelihood(stats_t *stat, vector_t *var_set, const char *refseq, const int refseq_length, 
    ...
    char *altseq = NULL;
    if (has_indel || dp)
        altseq = construct_altseq(refseq, refseq_length, stat->combo, var_data, &altseq_length);
    ...
    for (readi = 0; readi < nreads; readi++)
    {
        ...
        if (dp)
        {
            prgu = calc_prob_dp(readprobmatrix, read_data[readi]->length, refseq, refseq_length, read_data[readi]->pos, 
                                read_data[readi]->splice_pos, read_data[readi]->splice_offset, 
                                read_data[readi]->n_splice, gap_op, gap_ex, seqnt_map);
            prgv = calc_prob_dp(readprobmatrix, read_data[readi]->length, altseq, altseq_length, read_data[readi]->pos, 
                                read_data[readi]->splice_pos, read_data[readi]->splice_offset, 
                                read_data[readi]->n_splice, gap_op, gap_ex, seqnt_map);
        }
        else if (has_indel)
        {
            prgu = calc_prob(readprobmatrix, read_data[readi]->length, refseq, refseq_length, read_data[readi]->pos, 
                            read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
...
static void calc_likelihood_bisulfite(stats_t *stat, vector_t *var_set, const char *refseq, const int refseq_length, 
    ...
    for (readi = 0; readi < nreads; readi++)
    {
        ...
        count_v_usage += v_usage;v_usage=0;
        ...
        if (prgv1[0] > read_data[readi]->prgv)
        {
            ...
            read_data[readi]->prgv = (float)prgv1[0];
            read_data[readi]->pout = (float)pout;
        }

        /* Mixture model: heterozygosity or heterogeneity as explicit allele frequency mu such that P(r|GuGv) = (mu)(P(r|Gv)) + (1-mu)(P(r|Gu)) */
        double phet = log_add_exp(LOG50 + prgv1[0], LOG50 + prgu1[0]);
        double phet10 = log_add_exp(LOG10 + prgv1[0], LOG90 + prgu1[0]);
        double phet90 = log_add_exp(LOG90 + prgv1[0], LOG10 + prgu1[0]);
        if (phet10 > phet)
            phet = phet10;
        if (phet90 > phet)
            phet = phet90;
...
static char *evaluate(vector_t *var_set)
{
    ...
    if(methylation){
        ...
        // total = sum of stat[i]->mut + stat[i]->ref
...
static void process(const vector_t *var_list, FILE *out_fh)
{
    ...
    picked_ref = "chrM";
    fasta_t *f = refseq_fetch(picked_ref, fa_file);
    if (f == NULL)
        return NULL;
    char *refseq = f->seq;
    int refseq_length = f->seq_length;
