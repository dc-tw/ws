...
void set_prob_matrix(double *matrix, const read_t *read, const double *is_match, const double *no_match, 
                    const int *seqnt_map, const int bisulfite) {//is&no match意義? 這兩個array基本上是同一個數值 除了編號0與n-1(頭尾)可能不同
                    //seqnt_map意義?依據可能的char數目設置二維陣列 在init後設 matrix意義?
    ...
    for (b = 0; b < read->length; b++) {//對所有read搜尋
        ...
        if (bisulfite > 0) {
            ...
            case 'A':
                ...
                //matrix[read->length * seqnt_map['a' - 'A' + C-A] + b] = is_match[b]; // unmethylated reverse strand
                break;
            case 'T':
            ...
            if ((bisulfite == 1) && (read->qseq[b] == 'T') && ((!read->is_read2 && !read->is_reverse) || 
            (read->is_read2 && read->is_reverse))) 
                matrix[read->length * seqnt_map['C' - 'A'] + b] = is_match[b]; 
                // unmethylated forward strand, top strand

            else if ((bisulfite == 2) && (read->qseq[b] == 'A') && 
            ((!read->is_read2 && read->is_reverse) || (read->is_read2 && !read->is_reverse))) 
                matrix[read->length * seqnt_map['G' - 'A'] + b] = is_match[b]; 
                // unmethylated reverse strand, bottom strand

            else if ((bisulfite >= 3) && (read->qseq[b] == 'T') && 
            ((!read->is_read2 && !read->is_reverse) || (read->is_read2 && read->is_reverse))) 
                matrix[read->length * seqnt_map['C' - 'A'] + b] = is_match[b]; 
                // unmethylated forward strand, top strand

            else if ((bisulfite >= 3) && (read->qseq[b] == 'A') && 
            ((!read->is_read2 && read->is_reverse) || (read->is_read2 && !read->is_reverse))) 
                matrix[read->length * seqnt_map['G' - 'A'] + b] = is_match[b]; 
                // unmethylated reverse strand, bottom strand
        }
...
double calc_read_prob_bisulfite(char* var_list, double *matrix, int read_length, const char *seq, int seq_length, 
    ...
    if(alt == 1){//altseq
        ...
        {//chr pos ref alt
            ...
            else priority = (v->ref == read[pos])? 1: -1;
...
double calc_prob_region_bisulfite(char* var_list, double *matrix, int read_length, const char *seq, int seq_length, 
    ...
    for (i = start; i < end; i++) {//for each start position, do the calculation in lower floor
        p[i - start] = calc_read_prob_bisulfite(char* var_list, matrix, read_length, seq, seq_length, i, seqnt_map, alt, strand);
...
double calc_prob_bisulfite(char* var_list, const double *matrix, int read_length, const char *seq, int seq_length, 
    ...
    if (n_splice == 0) {
        probability = calc_prob_region_bisulfite(matrix, read_length, seq, seq_length, pos, start, end, seqnt_map, alt, strand);
    }
        ...
        for (i = 0; i <= n_splice; i++) {
            ...
            probability += calc_prob_region_bisulfite(submatrix, r_len, seq, seq_length, g_pos, start, end, seqnt_map, alt, strand);
...
double smith_waterman_gotoh(const double *matrix, int read_length, const char *seq, int seq_length, 
...
double calc_prob_dp(const double *matrix, int read_length, const char *seq, 
